...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        try:
            ...
            full_local_path = os.path.realpath(local_dir)
            if not local_dir.startswith('/'):
                full_local_path = os.path.realpath(os.path.join(artifact_path, local_dir))
            artifacts = {c['name']: self.process_artifact(full_local_path, c, version) for c in cfg['artifacts']}
    ...
    def process_artifact(artifact_path, artifact_info, version):
        ...
        local_file_name = os.path.join(artifact_path, os.path.basename(artifact_info['path_in_repo']))
