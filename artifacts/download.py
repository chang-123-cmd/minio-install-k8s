...
class DownloadHelper(object):
    ...
    def download(self, path_in_repo: str, local_file: str, progress=""):
        ...
        print("#   => " + local_file)
        print("#" * star_count)
        download_dir = os.path.dirname(local_file)
...
class Artifact(object):
    def __init__(self, item_cfg: dict, version: str):
        self.group = item_cfg.get('group', '')  # type: str
...
class Config(object):
    def __init__(self, config_file: str, selected_groups: set):
        self.name = ''
        self.version = ''
        self.local_root_dir = ''
        ...
        if selected_groups:
            self.artifacts = [artifact for artifact in self.artifacts if artifact.group in selected_groups]

    def __load_config(self, artifact_config_file):
        ...
        with open(str(cfg_filename)) as f:
            ...
            self.local_root_dir = os.path.realpath(local_dir)
            if not local_dir.startswith('/'):
                self.local_root_dir = os.path.realpath(os.path.join(os.path.dirname(cfg_filename), local_dir))
            for artifact_cfg in cfg['artifacts']:
                artifact = Artifact(artifact_cfg, self.version)
                artifact.local_file = os.path.join(self.local_root_dir, artifact.filename)
...
def parse_args():
    ...
    parser.add_argument('--resolve-only', '-r', action='store_true',
                        help='Just resolve, do not download.')
    parser.add_argument('--groups', '-g', type=str, default=None,
                        help='Download artifacts in selected groups.')

    return parser.parse_args()


def main(args: Namespace):
    selected_groups = set(cmd_args.groups.split(',')) if cmd_args.groups else None

    script_path = os.path.dirname(os.path.realpath(__file__))
    config = Config(os.path.join(script_path, "config.yml"), selected_groups)
    # print(config)
    if args.resolve_only:
        ...
        if selected_groups:
            print("selected groups: {}".format(selected_groups))
        download(config)
